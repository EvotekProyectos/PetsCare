...
<div class="row padding-1 p-1">
    <div class="col-md-12">
        ...
        <div class="col-md-6">
            ...
            value="{{ old('food', $hotel?->food) }}"  id="food" placeholder="Agregar tipo de alimentos, horarios de comida, indicaciones">{{ old('food', $lastHotel?->food) }}</textarea> 
            ...
            value="{{ old('objects', $hotel?->objects) }}" id="objects" placeholder="Lista de objetos que acompañan a la mascota">{{ old('objects', $lastHotel?->objects) }}</textarea>
            </div>
    ...
    <div class="row">
        <div class="col-md-6">
            ...
            class="form-control @error('observations') is-invalid @enderror"
             value="{{ old('observations', $hotel?->observations) }}" id="observations" placeholder="Detallar las necesidades particulares de la mascota">{{ old('observations', $lastHotel?->observations) }}</textarea>
                        ...
                        {{-- <select name="service_type_id" class="form-control2 @error('service_type_id') is-invalid @enderror" id="service_type_id">
                            ...
                            @foreach ($products as $product)
                                <option value="{{ $product->ARTICULO_ID }}" name="service_type_id"
                                    ...
                                    {{ old('service_type_id', $lastHotel?->serv->NOMBRE) }}
                                    {{ $product->NOMBRE }}
                                </option>
                            @endforeach
                        </select> --}}
                        <select name="service_type_id" class="form-control2 @error('service_type_id') is-invalid @enderror" id="service_type_id">
                            ...
                            @foreach ($products as $product)
                                <option value="{{ $product->ARTICULO_ID }}"
                                    {{ old('service_type_id', $hotel?->service_type_id ?? $lastHotel?->service_type_id) == $product->ARTICULO_ID ? 'selected' : '' }}>
                                    {{ $product->NOMBRE }}
                                </option>
                            @endforeach
                        </select>
        ...
        <div class="row">
            ...
            <div class="input-group mb-3">
                    ...
                    <select name="cubicle_id" class="form-control @error('cubicle_id') is-invalid @enderror" id="cubicle_id">
                        <option value="">Selecciona el número de cubículo</option>
                        @foreach($cubicles as $cubicle)
                            <option value="{{ $cubicle->id }}"
                                {{ old('cubicle_id', $hotel?->cubicle_id ?? $lastHotel?->cubicle_id) == $cubicle->id ? 'selected' : '' }}>
                                {{ $cubicle->name }}
                            </option>
                        @endforeach
                    </select>
...
{{-- 
      ...
      <div class="col-12 mt-2 d-flex justify-content-end">
        <button id="btn-agregar-servicio"  type="submit" class="btn btn-primary btn-sm text-uppercase rounded-4 ">
            <i class="fas fa-plus"></i>
            Agregar servicio</button>
    </div>  --}}

    <div class="col-12 mt-2 d-flex justify-content-end">
        <button type="submit" class="btn-agregar-servicio btn btn-primary btn-sm text-uppercase rounded-4">
            <i class="fas fa-plus"></i>
            Agregar servicio
        </button>
    </div>
