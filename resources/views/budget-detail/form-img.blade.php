<div class="row padding-1 p-1">
    <div class="row">
        ...
        <div class="form-group mb-2 mb20" hidden>
            <label for="budget_id_img" class="form-label">{{ __('Budget Id') }}</label>
            <input type="text" name="budget_id" class="form-control @error('budget_id') is-invalid @enderror"
                value="{{ old('budget_id', $budget?->id) }}" id="budget_id_img" placeholder="Budget Id">
        ...
        <div class="col-5 form-group mb-2 mb20">
            <label for="img_id" class="form-label">IMAGENES</label>
            <div class="input-group mb-3">
                <span class="input-group-text bg-primary-subtle" id="basic-addon1">
                    <span class="vaadin--lines-list"></span></span>
                <select name="img_id" class="form-control @error('img_id') is-invalid @enderror"
                    id="img_id" onchange="getimgprice(this.value)">
                    <option value=""> Selecciona el servico a registrar</option>
                    @foreach ($products as $product)
                        {{-- @if ($product->product_classification_id == 4) --}}
                        <option value="{{ $product->ARTICULO_ID }}" name="img_id"
                            {{ old('img_id', $budgetDetail?->img_id) == $product->ARTICULO_ID ? 'selected' : '' }}>
                            {{ $product->NOMBRE }}</option>
                        {{-- @endif  --}}
                    @endforeach
                </select>
            </div>
            {!! $errors->first('img_id', '<div class="invalid-feedback" role="alert"><strong>:message</strong></div>') !!}
        </div>
        <div class="col-3 form-group mb-2 mb20">
            <div class="form-group mb-2 mb20">
                <label for="priceimg" class="form-label">Precio</label>
                <div class="input-group mb-3">
                    <span class="input-group-text bg-primary-subtle" id="basic-addon2">
                        <span class="f7--money-dollar"></span>
                    </span>
                    <input type="text" name="price" class="form-control @error('price') is-invalid @enderror"
                        value="{{ old('price', $budgetDetail?->price) }}" id="priceimg" placeholder="Precio">
                </div>
        ...
        <div class="col-3 form-group mb-2 mb20">
            <div class="form-group mb-2 mb20">
                <label for="notesimg" class="form-label">Notas</label>
                <div class="input-group mb-3">
                    <span class="input-group-text bg-primary-subtle" id="basic-addon2">
                        <span class="vaadin--lines-list"></span>
                    </span>
                    <input type="text" name="notes" class="form-control @error('notes') is-invalid @enderror"
                        value="{{ old('notes', $budgetDetail?->notes) }}" id="notesimg" placeholder="Notas">
                </div>
