...
<body>
    ...
    <form id="formVale">
        ...
        <div class="section-title">MOTIVO DE CANCELACIÓN</div>
        <div class="motivo-box">
            @if (!isset($isPdf) || !$isPdf)
                <textarea name="observaciones" rows="4" placeholder="Escriba el motivo de cancelación..."
                    class="textarea-observaciones" style="width: 100%;"></textarea>
            @else
        ...
        @if ($isPdf ?? false)
            <table style="width:100%; border-collapse:collapse; margin-top:40px;">
                <tr>
                    ...
                    <td style="width:50%; text-align:center; vertical-align:bottom; padding: 0 20px;">
                        @if ($voucher->cancellation_signature)
                            <img src="{{ public_path('storage/' . $voucher->cancellation_signature) }}"
                                style="width:300px;height:120px;object-fit:contain;">
                        @endif
                        <div class="firma-line">
                            <b>CANCELADO POR</b><br>
                            {{ $voucher->cancelled->name }}
                        </div>
                    </td>
                </tr>
            </table>
    ...
    <script src="{{ asset('js/vouchers/cancel.js') }}" defer></script>
    <script>
    ...
    <script>
        const CANCEL_VOUCHER_URL = "{{ route('vouchers.cancel', $voucher->id) }}";
    </script>
</body>
